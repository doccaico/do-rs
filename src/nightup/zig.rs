...
pub fn run(dist_dir: &str, download_dir: &str) -> Result<()> {
    let json_url = "https://ziglang.org/download/index.json";

    // curlを実行して index.json を取得
    ...
    // ダウンロードURLを取得する
    let re_download_url =
        Regex::new(r#"(?ms)"master": \{.*?"x86_64-windows": \{.*?"tarball": "(.+?)""#)
            .context("failed to compile zig.re_download_url (regex)")?;

    let download_url = re_download_url
        .captures(&contents)
    ...
    let local_zip_name = "zig-latest.zip";
